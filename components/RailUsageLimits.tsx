...
const MARKS: Record<ProviderLimitId, { viewBox: string; path: string }> = {
  codex: {
    viewBox: "0 0 24 24",
...
const TONE_CLASS: Record<ReturnType<typeof limitTone>, string> = {
...
function ProviderMark({ id }: { id: ProviderLimitId }) {
  const mark = MARKS[id];
  return (
    <svg
...
function Meter({ provider }: { provider: ProviderLimit }) {
  const windows =
    provider.windows.length > 0 ? provider.windows : [null];
  return (
    <span
      className="inline-flex shrink-0 items-center gap-0.5"
      title={providerLimitLabel(provider)}
      aria-label={providerLimitLabel(provider)}
    >
      ...
      <span className="inline-flex items-center gap-1 font-semibold tabular-nums">
        {windows.map((window, index) => {
          ...
          return (
            <span
              key={`${provider.providerId}-${window?.label ?? index}`}
              className={TONE_CLASS[limitTone(remaining)]}
            >
...
/**
 * Compact provider usage meters for the Chief rail — same figures as
 * sidebar-pro's top-row slot, shown only when that sidebar is hidden.
 */
export function RailUsageLimits() {
  const rpc = useRpc<typeof rpcContract>();
  const [providers, setProviders] = useState<ProviderLimit[]>(emptyProviderLimits);

  const refresh = useCallback(async () => {
    try {
      const result = await rpc.call("providerLimits", {});
  ...
  return (
    <div
      className="flex shrink-0 flex-wrap items-center gap-2 border-t border-border bg-background px-1 pt-2 text-[12px] leading-none text-foreground"
      data-testid="chief-rail-usage-limits"
      aria-label="Provider usage limits"
    >
      {providers.map((provider) => (
        <Meter key={provider.providerId} provider={provider} />
      ))}
    </div>
  );
}
